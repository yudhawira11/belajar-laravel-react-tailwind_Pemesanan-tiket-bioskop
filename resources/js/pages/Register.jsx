...
const Register = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            setError(err.response?.data?.message || 'Gagal daftar. Periksa data kamu.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="mx-auto w-full max-w-md rounded-3xl border border-white/60 bg-white/80 p-8 shadow-[0_25px_60px_-45px_rgba(15,23,42,0.35)] dark:border-slate-800/80 dark:bg-slate-900/70">
            <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Buat Akun Baru</h1>
            <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                Daftar untuk menikmati tiket bioskop digital.
            </p>
            ...
            <p className="mt-6 text-sm text-slate-600 dark:text-slate-300">
                Sudah punya akun?{' '}
                <Link to="/login" className="font-semibold text-slate-900 underline dark:text-white">
                    Login di sini
                </Link>
            </p>
        </div>
